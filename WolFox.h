...
typedef NS_ENUM(NSInteger, WFLogCategory) {
    WFLogCore, WFLogLocation, WFLogMovement, WFLogRandom,
    WFLogRoute, WFLogWiFi, WFLogDevice, WFLogScheduler,
    WFLogStorage, WFLogUI
};
...
- (void)setDictionaryForKey:(NSString *)key value:(NSDictionary *)value;
...
- (WFError *)startFromLatitude:(double)fromLat longitude:(double)fromLon
                ...
                updateInterval:(NSTimeInterval)intervalSeconds
                        onTick:(void (^)(double lat, double lon, double course,
                                         double remaining, double progress))onTick
...
- (void)initialize;
- (WFError *)activateStaticLocationWithLatitude:(double)latitude longitude:(double)longitude;
...
// Phases 4–9: NOT YET IMPLEMENTED — return WFErrorCodeNotAvailable honestly.
